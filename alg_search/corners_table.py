    ...
    def page_list(action: str, value_ent: Entry = 1):
        ...
        if(action == "start"):
            for val in range((page-1)*values , page*values+1):
                my_page[0] = my_page[0] + f"sin({val}) = {round(math.sin(math.radians(val)),2)}\n"
                my_page[1] = my_page[1] + f"cos({val}) = {round(math.cos(math.radians(val)),2)}\n"
                my_page[2] = my_page[2] + f"tan({val}) = {round(math.tan(math.radians(val)),2)}\n"
                if math.sin(val) == 0:
                    my_page[3] = my_page[3] + f"ctg({val}) = - \n"
                else:
                    my_page[3] = my_page[3] + f"ctg({val}) = {round(math.cos(math.radians(val))/math.sin(math.radians(val)), 2)}\n"
